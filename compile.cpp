	...
	for (i=0;i<=255;i++)//stack
	{
		for (j=0;j<=255;j++)//x
		{
			for (k=0;k<=255;k++)//y
			{
				...
				{
					if (i*l <= 255)//pop z;push z (l-1 times);add (l-1 times);
					{
						d[i*l][j][k] = min(d[i*l][j][k],d[i][j][k]+2*l-1);
					}
				...
				if (i+j<=255)//push x;add;pop x;push x;
				{
					d[i+j][i+j][k] = min(d[i+j][i+j][k],d[i][j][k]+4);
				}
				if (i+k<=255)//same
				{
					d[i+k][j][i+k] = min(d[i+k][j][i+k],d[i][j][k]+4);
				}
